...
class Postgres(object):
    ...
    def generate_postgres_init_sql(self):
        ...
        if self.settings.get("CN_INSTALL_GLUU_GATEWAY") == "Y":
    ...
    def create_patch_secret_init_sql(self):
        ...
        self.kubernetes.patch_or_create_namespaced_secret(name="pg-init-sql",
                                                          namespace=self.settings.get("CN_POSTGRES_NAMESPACE"),
                                                          literal="data.sql",
    ...
    def patch_or_install_postgres(self):
        ...
        if self.settings.get("CN_JACKRABBIT_CLUSTER") == "N":
            ...
            self.kubernetes.patch_namespaced_stateful_set_scale(name="postgres",
                                                                replicas=3,
                                                                namespace=self.settings.get("CN_POSTGRES_NAMESPACE"))
            self.kubernetes.check_pods_statuses(self.settings.get("CN_POSTGRES_NAMESPACE"), "app=postgres", self.timeout)

    def install_postgres(self):
        self.uninstall_postgres()
        self.kubernetes.create_namespace(name=self.settings.get("CN_POSTGRES_NAMESPACE"), labels={"app": "postgres"})
        self.create_patch_secret_init_sql()
        ...
        postgres_parser["spec"]["replicas"] = self.settings.get("CN_POSTGRES_REPLICAS")
        postgres_parser["spec"]["monitor"]["prometheus"]["namespace"] = self.settings.get("CN_POSTGRES_NAMESPACE")
        if self.settings.get("CN_DEPLOYMENT_ARCH") == "local":
            postgres_parser["spec"]["storage"]["storageClassName"] = "openebs-hostpath"
        postgres_parser["metadata"]["namespace"] = self.settings.get("CN_POSTGRES_NAMESPACE")
        if self.settings.get("CN_DEPLOYMENT_ARCH") in ("microk8s", "minikube") or \
                self.settings.get("CN_TEST_ENVIRONMENT") == "Y":
            try:
        ...
        self.kubernetes.create_namespaced_custom_object(filepath=postgres_yaml,
                                                        ...
                                                        plural="postgreses",
                                                        namespace=self.settings.get("CN_POSTGRES_NAMESPACE"))
        if not self.settings.get("CN_AWS_LB_TYPE") == "alb":
            self.kubernetes.check_pods_statuses(self.settings.get("CN_POSTGRES_NAMESPACE"), "app=postgres", self.timeout)
    ...
    def uninstall_postgres(self):
        ...
        self.kubernetes.delete_service("kubedb", self.settings.get("CN_POSTGRES_NAMESPACE"))
        self.kubernetes.delete_service("postgres", self.settings.get("CN_POSTGRES_NAMESPACE"))
        self.kubernetes.delete_service("postgres-replicas", self.settings.get("CN_POSTGRES_NAMESPACE"))
        self.kubernetes.delete_service("postgres-stats", self.settings.get("CN_POSTGRES_NAMESPACE"))
