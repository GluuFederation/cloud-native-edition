...
class Redis(object):
    ...
    def install_redis(self):
        ...
        self.kubernetes.create_namespace(name=self.settings.get("CN_REDIS_NAMESPACE"), labels={"app": "redis"})
        ...
        redis_conf_parser["metadata"]["namespace"] = self.settings.get("CN_REDIS_NAMESPACE")
        ...
        redis_parser["spec"]["cluster"]["master"] = self.settings.get("CN_REDIS_MASTER_NODES")
        redis_parser["spec"]["cluster"]["replicas"] = self.settings.get("CN_REDIS_NODES_PER_MASTER")
        redis_parser["spec"]["monitor"]["prometheus"]["namespace"] = self.settings.get("CN_REDIS_NAMESPACE")
        if self.settings.get("CN_DEPLOYMENT_ARCH") == "local":
            redis_parser["spec"]["storage"]["storageClassName"] = "openebs-hostpath"
        redis_parser["metadata"]["namespace"] = self.settings.get("CN_REDIS_NAMESPACE")
        ...
        self.kubernetes.create_namespaced_custom_object(filepath=redis_yaml,
                                                        ...
                                                        version="v1alpha1",
                                                        plural="redises",
                                                        namespace=self.settings.get("CN_REDIS_NAMESPACE"))
    ...
    def uninstall_redis(self):
        ...
        self.kubernetes.delete_service("kubedb", self.settings.get("CN_REDIS_NAMESPACE"))
