...
{{/*
    Common labels
*/}}
{{- define "oxd-server.labels" -}}
app: {{ include "oxd-server.name" . }}
helm.sh/chart: {{ include "oxd-server.chart" . }}
...
*/}}
{{- define "oxd-server.usr-secret-envs"}}
...
- maxSkew: {{ $val.maxSkew }}
  ...
  labelSelector:
    matchLabels:
      app: {{ $.Release.Name }}-{{ include "oxd-server.name" $ }}
