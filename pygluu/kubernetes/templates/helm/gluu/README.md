...
| config.configmap.gluuRadiusEnabled | bool | `false` | Boolean flag to enable/disable Radius chart |
...
| global.oxauth.enabled | bool | `true` | Boolean flag to enable/disable auth-server chart. You should never set this to false. |
...
| radius.image.tag | string | `"4.3.0_b1"` | Image  tag to use for deploying |
...
| radius.livenessProbe.exec | object | `{"command":["cat","/tmp/gluu-radius-health"]}` | Executes interal command healthcheck. |
...
| radius.readinessProbe.exec | object | `{"command":["cat","/tmp/gluu-radius-health"]}` | Executes interal command healthcheck. |
