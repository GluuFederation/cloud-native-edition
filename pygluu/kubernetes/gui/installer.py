...
class InstallHandler(object):
    ...
    def upgrade_values_yaml(self):
        try:
            ...
            helm = Gluu()
            if gluu_settings.db.get("CN_JACKRABBIT_CLUSTER") == "Y":
    ...
    def install(self):
        try:
            ...
            helm = Gluu()
            if gluu_settings.db.get("CN_INSTALL_REDIS") == "Y" or \
                    gluu_settings.db.get("CN_INSTALL_GLUU_GATEWAY") == "Y" or \
                    gluu_settings.db.get("CN_JACKRABBIT_CLUSTER") == "Y":
                from pygluu.kubernetes.kubedb import Kubedb
                kubedb = Kubedb()
                kubedb.uninstall_kubedb()
                kubedb.install_kubedb()
            ...
            if gluu_settings.db.get("CN_INSTALL_REDIS") == "Y":
